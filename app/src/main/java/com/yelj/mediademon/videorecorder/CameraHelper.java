...
public class CameraHelper {
    ...
    public static Camera.Size getOptimalVideoSize(List<Camera.Size> supportedVideoSizes, List<Camera.Size> previewSizes, int w, int h) {
        ...
        // TODO: 2017/4/25 by ylj
        ArrayList<Camera.Size> supportSizes = new ArrayList<>();
        //升序排列
        Collections.sort(videoSizes, new AscOrder());
        int currentHeight = 0;
        for (int i = 0; i < videoSizes.size(); i++) {
            Camera.Size size = videoSizes.get(i);
            if (size.height >= 720) {
                if (currentHeight != 0 && size.height != currentHeight) {
                    break;
                }
        ...
        for (Camera.Size size : supportSizes) {
            double ratio = (double) size.width / size.height;
            if (Math.abs(ratio - targetRatio) <= minDiff && previewSizes.contains(size)) {
                optimalSize = size;
    ...
    private static class AscOrder implements Comparator<Camera.Size> {
